...
void setup() {
  ...
  int gridWidth = ceil(width/xd) + 1;
  int gridHeight = ceil(height/yd) + 1;
  for (int i = 0; i < gridHeight; i++) {
    for (int j = 0; j < gridWidth; j++) {
      float x = j * xd;
      float y = i * yd;
      float cubeSize = constrain(map(noise(x*noiseVar, y*noiseVar), 0, 1, -0.7*maxCubeSize, 1.8*maxCubeSize), 0, maxCubeSize);
